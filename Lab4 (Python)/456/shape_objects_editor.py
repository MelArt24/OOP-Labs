...
class ShapeObjectsEditor(Shape, ShapeEditor):
    def __init__(self, root, canvas, shapes, toolbar):
        ...
        self.icon1 = tk.PhotoImage(file="C:/Users/User/Desktop/Матеріали для університету/2 курс 1 "
                                        "семестр/ООП/Лабораторні роботи/Лаба 3/Point.png")
        self.icon2 = tk.PhotoImage(file="C:/Users/User/Desktop/Матеріали для університету/2 курс 1 "
                                        "семестр/ООП/Лабораторні роботи/Лаба 3/Line.png")
        self.icon3 = tk.PhotoImage(file="C:/Users/User/Desktop/Матеріали для університету/2 курс 1 "
                                        "семестр/ООП/Лабораторні роботи/Лаба 3/Rectangle.png")
        self.icon4 = tk.PhotoImage(file="C:/Users/User/Desktop/Матеріали для університету/2 курс 1 "
                                        "семестр/ООП/Лабораторні роботи/Лаба 3/Ellipse.png")
        self.icon5 = tk.PhotoImage(file="C:/Users/User/Desktop/Матеріали для університету/2 курс 1 "
        ...
        # BUTTON 5 (cube)
        self.button5_state = False

        def toggle_button5():
            if self.button5_state:
                self.button5_state = False
                self.button5.config(relief=tk.RAISED)
                self.selected_shape_var.set(0)
                self.current_editor = None
            else:
                self.button5_state = True
                self.button5.config(relief=tk.SUNKEN)
                self.selected_shape_var.set(5)
                self.set_cube_editor()

        self.button5 = self.create_tool_button(self.icon1, "Cube", toggle_button5)

        # BUTTON 6 (line_oo_shape)
        self.button6_state = False

        def toggle_button6():
            if self.button6_state:
                self.button6_state = False
                self.button6.config(relief=tk.RAISED)
                self.selected_shape_var.set(0)
                self.current_editor = None
            else:
                self.button6_state = True
                self.button6.config(relief=tk.SUNKEN)
                self.selected_shape_var.set(6)
                self.set_lineooshape_editor()

        self.button6 = self.create_tool_button(self.icon2, "LineOOShape", toggle_button6)
    ...
    def create_menus(self):
        ...
        shapes_menu.add_radiobutton(label="Cube", variable=self.selected_shape_var, value=5,
                                    command=lambda: self.set_cube_editor())
        shapes_menu.add_radiobutton(label="LineOOShape", variable=self.selected_shape_var, value=6,
                                    command=lambda: self.set_lineooshape_editor())
    ...
    def set_cube_editor(self):
        self.clear_button_states()
        self.current_editor = CubeEditor(self.canvas, self.shapes)
        self.button5_state = True
        self.button5.config(relief=tk.SUNKEN)
        self.selected_shape_var.set(5)

    def set_lineooshape_editor(self):
        self.clear_button_states()
        self.current_editor = LineOOShapeEditor(self.canvas, self.shapes)
        self.button6_state = True
        self.button6.config(relief=tk.SUNKEN)
        self.selected_shape_var.set(6)
