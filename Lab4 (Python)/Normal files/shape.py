...
class Cube(Line, Rectangle):
    ...
    def draw(self, canvas, center_x, center_y, end_x, end_y):
        # center_x = center_x if center_x is not None else self.center_x
        # center_y = center_y if center_y is not None else self.center_y
        # end_x = end_x if end_x is not None else self.end_x
        # end_y = end_y if end_y is not None else self.end_y
        ...
        print(f"second_top_x: {second_top_x}")
    ...
    #     return (f"Cube (center=({self.center_x}, {self.center_y}), end=({self.end_x}, {self.end_y}), "
