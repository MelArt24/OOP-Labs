...
def show_about():
    messagebox.showinfo("About", "Lab5 version 1.0")
...
def main():
    ...
    editor2 = MyEditor(root, canvas, shapes, toolbar, selected_shape_var)

    print(editor is editor2)
    print(editor)
    print(editor2)
    ...
    button_open_table = tk.Button(toolbar, text="Open Table", command=editor.open_table)
    button_open_table.pack(side=tk.LEFT, padx=5)
    editor.ToolTip(button_open_table, "Open Table")

    save_button = tk.Button(toolbar, text="Save", command=lambda: editor.save_shapes_to_file("lab5_oop.txt"))
    save_button.pack(side=tk.LEFT)
    editor.ToolTip(save_button, "Save")
